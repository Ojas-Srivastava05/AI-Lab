...
def steepest_ascent_hc(board):
    ...
    return initial_h, current_h, steps, (current_h == 0)
...
def random_restart_hc(max_restarts=100):
    total_steps = 0
    restarts = 0
    initial_h_first = None

    for _ in range(max_restarts):
        board = generate_random_board()
        init_h, final_h, steps, solved = steepest_ascent_hc(board)

        if initial_h_first is None:
            initial_h_first = init_h

        total_steps += steps
        restarts += 1

        if solved:
            return initial_h_first, final_h, total_steps, True, restarts

    return initial_h_first, final_h, total_steps, False, restarts
...
def run_experiment(algorithm_name, algorithm_func, num_trials=50):
    ...
    for i in range(num_trials):
        ...
        if algorithm_name == "Random-Restart HC":
            init_h, final_h, steps, solved, restarts = random_restart_hc()
            results.append({
                'trial': i + 1,
                'initial_h': init_h,
                'final_h': final_h,
            ...
            init_h, final_h, steps, solved = algorithm_func(board)
            results.append({
                'trial': i + 1,
                'initial_h': init_h,
                'final_h': final_h,
...
def print_results(algorithm_name, results):
    ...
    if algorithm_name == "Random-Restart HC":
        print(f"{'Trial':<7} {'Init h':<9} {'Final h':<9} {'Steps':<8} {'Restarts':<10} {'Status'}")
        print("-" * 75)
        for r in results:
            status = "SOLVED" if r['solved'] else "FAIL"
            print(f"{r['trial']:<7} {r['initial_h']:<9} {r['final_h']:<9} {r['steps']:<8} {r['restarts']:<10} {status}")
    else:
        print(f"{'Trial':<7} {'Init h':<9} {'Final h':<9} {'Steps':<8} {'Status'}")
        print("-" * 75)
        for r in results:
            status = "SOLVED" if r['solved'] else "FAIL"
            print(f"{r['trial']:<7} {r['initial_h']:<9} {r['final_h']:<9} {r['steps']:<8} {status}")
...
def print_summary(all_results):
    ...
    print("=" * 80)
    print(f"{'Algorithm':<25} {'Solved':<10} {'Success%':<12} {'Avg Steps':<12} {'Avg Final h'}")
    print("-" * 80)
    ...
    for name, results in all_results.items():
        ...
        avg_final_h = sum(r['final_h'] for r in results) / total
        print(f"{name:<25} {solved}/{total:<7} {pct:<12.1f} {avg_steps:<12.2f} {avg_final_h:.2f}")
...
def main():
  ...
  1. STEEPEST-ASCENT HILL CLIMBING:
     - Evaluates ALL neighbors at each step and picks the best.
     - Often gets stuck in local minima (typically ~14% success rate).
     - Fast per run but frequently fails.

  2. FIRST-CHOICE HILL CLIMBING:
     - Generates random successors until a better one is found.
  ...
  3. RANDOM-RESTART HILL CLIMBING:
     ...
     - Overcomes local minima by restarting from different initial states.

  4. SIMULATED ANNEALING:
     - Allows uphill moves with decreasing probability (temperature schedule).
     - Can escape local minima, leading to higher success rates.
  ...
  KEY INSIGHT:
     ...
     local minima. Random restarts solve this by trying many starting points.
